...
public class fileUp : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        string fileExts = ConfigurationManager.AppSettings["fileExt"].ToString();
        string[] filetype = fileExts.Split(',');    //文件允许格式    
        int size = 500;   //文件大小限制,单位MB,同时在web.config里配置环境默认为100MB
