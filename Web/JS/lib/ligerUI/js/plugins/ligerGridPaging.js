        ...
        */
        ...
        changePage: function (ctype)
        {
            ...
            if (p.dataAction != "local" && g.isDataChanged && !confirm(p.isContinueByDataChanged))
                return false;
            p.pageCount = MaxPage//parseInt($(".pcontrol span", g.toolbar).html());
            switch (ctype)
            {
        ...
        _getCurrentPageData: function (source)
        {
            ...
            if (!p.newPage) p.newPage = 1;
            for (i = (1 - 1) * p.pageSize; i < source[p.root].length && i < p.newPage * p.pageSize; i++)
            {
        ...
        {
            var g = this, p = this.options;
            $('.pcontrol input', g.toolbar).val(PageIndex);
            if (!p.pageCount) p.pageCount = 1;
            $('.pcontrol span', g.toolbar).html((MaxPage == 0 ? 1 : MaxPage)); //p.pageCount
            var r1 = parseInt((PageIndex - 1) * p.pageSize) + 1.0;
