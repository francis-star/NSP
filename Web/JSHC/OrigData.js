...
function InitControl() {
    ...
    /********权限******/
    GetMenuRight(toptoolbarManager, "2016112311926");
...
function Search() {
    ...
    GridLoadAll("BLL_OriginalData", "GetOriginalData", "GetOriginalDataCount", arr);
}
...
function View() {
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['65%', '70%'],
        content: '../Form/frm_View.htm?code=' + rows[0].OD_Code
    });

    //myShowModalDialog('frm_View.htm?code=' + rows[0].OD_Code, window, 800, 400);
}
...
function OrigDataDts(OD_Code, OD_TotleCount) {
    if (OD_TotleCount == 0) {
        ...
        top.layer.open({
            ...
            shade: 0.3,
            area: ['50%', '50%'],
            content: '../Form/frm_OrigDataDts.htm?code=' + OD_Code
        });
    }
}
...
function VaildData(OD_Code, OD_TotleCount) {
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['50%', '50%'],
        content: '../Form/frm_ValidData.htm?code=' + OD_Code
    });
}
...
function NOVaildData(OD_Code, OD_TotleCount) {
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['50%', '50%'],
        content: '../Form/frm_NOVaildData.htm?code=' + OD_Code
    });
}

//营销成功
function AlreadyUse(OD_Code, OD_TotleCount) {
    if (OD_TotleCount == 0) {
        $.ligerDialog.warn('未筛选，没有数据');
        return;
    }
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['50%', '50%'],
        content: '../Form/frm_AlreadyUseData.htm?code=' + OD_Code
    });
}
...
function ShowDialog() {
    top.layer.open({
        ...
        shade: 0.3,
        area: ['35%', '68%'],
        content: '../Form/frm_ImportData.htm',
        end: function () {
            InitData();
        }
    });
...
function ShowInvalidData() {
    top.layer.open({
        ...
        shade: 0.3,
        area: ['35%', '50%'],
        content: '../Form/frm_DInvalidData.htm',
        end: function () {
            InitData();
        }
    });
...
function DataScreen() {
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['65%', '70%'],
        content: '../Form/frm_DataScreen.htm?code=' + str,
        end: function () {
            InitData();
        }
    });
...
function MarkAlready() {
    ...
    $.ligerDialog.confirm('确认将此数据状态标记为“已使用”？', function (yes) {
        if (yes) {
            ...
            ReturnValue("BLL_OriginalData", "MarkAlreadyUse", arr, Address, "");
            InitData();
        }
    });
}
...
function DeleteDatas() {
    ...
    $.ligerDialog.confirm('确定删除该数据？', function (yes) {
        if (yes) {
            var arr = new Array();
            arr[0] = rows[0].OD_Code;
            ReturnMethod("BLL_OriginalData", "DeleteDatas", arr, Address, "DeleteSucess");
        }
    });
}
...
function UpdateDatas() {
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['30%', '45%'],
        content: '../Form/frm_UpdateOrigData.htm?Code=' + rows[0].OD_Code,
        end: function () {
            InitData();
        }
    });
