...
function InitControl() {
    ...
    GetMenuRight(toptoolbarManager, "2016112311944");
...
function Search() {
    ...
    MaxCount = ReturnValue("BLL_CustomerService", "GetWQTCustomerCount", arr, Address);
    MaxPage = (MaxCount % PageNum == 0 ? MaxCount / PageNum : Math.ceil((MaxCount / PageNum)));

    var arrStr = arr;
    ReplaceStr(arrStr);
    var Method = "GetWQTCustomer";
...
function Exam() {
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['85%', '75%'],
        content: '../../Form/WQT/frm_CustomerServiceAudit.htm?Code=' + row.Cust_Code + '&State=Exam', //iframe的url
        end: function () {
            Search();
        }
    });
...
, btn: ['确定', '取消']
  , yes: function (index, layero) {
      ...
      arr[1] = $("#selIsBill").val();
      arr[2] = $("#XGYY").val();
      var result = ReturnValue("BLL_CustomerService", "UpWQTBillHistory", arr, Address);
      if (result == "true") {
...
function View() {
    ...
    if (row) {
        ...
        top.layer.open({
            ...
            shade: 0.3,
            area: ['85%', '75%'],
            content: '../../Form/WQT/frm_CustomerServiceAudit.htm?Code=' + row.Cust_Code + '&State=View', //iframe的url
            end: function () {
                Search();
            }
        });
