...
function InitCount() {
    ...
    var Method = "GetWQTSimilarInfoCount";
...
function InitData() {
    var arr = new Array();
    arr[0] = Code;
    var oneData = eval(ReturnValue("BLL_CustomerService", "GetWQTCustomerByCode", arr, Address));
    ...
    if (oneData[0].Cust_CityName == "徐州市") {
        var message = oneData[0].Cust_Linkman + ": 您好! " + oneData[0].Cust_Name +
             " 已开通消费维权快速和解通道（www.wqkstd.com），用户名：" +
            oneData[0].Cust_BillNumber + "，密码：123,请妥善保管,若有和解单系统会发送短信通知。询:4000025315/4000085315";
        $("#txtMessage").val(message); 
    }
    else {
       var message = oneData[0].Cust_Linkman + ": 您好! " + oneData[0].Cust_Name +
              " 已开通消费维权快速和解通道（www.js96315.com），用户名：" +
            oneData[0].Cust_BillNumber + "，密码：123,请妥善保管,若有和解单系统会发送短信通知。询:4000025315/4000085315";
        $("#txtMessage").val(message);
    }
...
function Exam() {
    if (State == "Exam") {
        ...
        var result = ReturnValue("BLL_CustomerService", "PassWQT", arr, Address);
        if (result == "true") {
            if (arr[1] == "0") {
                layer.msg("审核成功");
                // 修改营销成功
                ...
                ReturnValue("BLL_CustomerService", "UpWQTAlreadyUse", arr1, Address);
...
function Search() {
    ...
    if (searchtype == "0") {//全部txtMessage
        ...
        var BLL = "BLL_CustomerService";
        var search_result = JSON.parse(ReturnValue("BLL_CustomerService", "GetWQTSimilarInfo", arr, Address));
        ...
        arr[2] = PageIndex;
        arr[3] = PageNum;
        var Method = "GetWQTNameContainsKeySimilarInfo";
        var BLL = "BLL_CustomerService";
        var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
        ...
        arr[2] = PageIndex;
        arr[3] = PageNum;
        var Method = "GetWQTBillNoSimilarInfo";
        var BLL = "BLL_CustomerService";
        var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
        ...
        arr[2] = PageIndex;
        arr[3] = PageNum;
        var Method = "GetWQTCustAddressSimilarInfo";
        var BLL = "BLL_CustomerService";
        var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
        ...
        var arrStr = arr;
        ReplaceStr(arrStr);
        var Method = "GetWQTCustPhoneSimilarInfo";
        var BLL = "BLL_CustomerService";
        var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
