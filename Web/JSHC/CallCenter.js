...
function InitControl() {
        ...
        columns: [
            {
                display: '', width: 30, render: function (rowdata, rowindex, value) {
                    var s = "";
                    if (rowdata.Cust_State == "退回" || rowdata.Cust_State == "异常退回") {
            ...
            { display: '开通时间', name: 'Cust_OpenDate', minWidth: 90 },
            {
                display: '地区', minWidth: 90, render: function (rowdata, rowindex, value) {
                    ...
                    //s = "<a href='#' title=" + rowdata.Cust_Address + " id='" + id + "' onmouseover='gg(this);' >" + rowdata.Cust_Area + "</a>";
    ...
    GetMenuRight(toptoolbarManager, "2016112311927");
}
...
function Search() {
    ...
    MaxCount = ReturnValue("BLL_CallCenter", "GetDataCount", arr, Address);
    MaxPage = (MaxCount % PageNum == 0 ? MaxCount / PageNum : Math.ceil((MaxCount / PageNum)));

    var arrStr = arr;
    ReplaceStr(arrStr);
    var Method = "GetCallCenterData";
...
function Add() {
    top.layer.open({
        ...
        shade: 0.3,
        area: ['70%', '80%'],
        content: '../Form/frm_CallCenterDts.htm?Code=',//iframe的url
        end: function () {
            Search();
        }
...
function Modify() {
    ...
    if (row) {
        if (row.Cust_State == "待审" || row.Cust_State == "退回" || row.Cust_State == "异常退回") {
            top.layer.open({
                type: 2,
                title: '修改客户单',
                shadeClose: false,
                shade: 0.3,
                area: ['70%', '80%'],
                content: '../Form/frm_CallCenterDts.htm?fromPage=CallCenter_A&Code=' + row.Cust_Code, //iframe的url
                end: function () {
                    Search();
                }
            });
    ...
    if (row) {
        ...
        $.ligerDialog.confirm('确定删除？', function (yes) {
            if (yes) {
                var arr = new Array();
                arr[0] = row.Cust_Code;
                var Data = ReturnValue("BLL_CallCenter", "DeleteCallCenterData", arr, Address);
                if (Data == "true") {
