...
$(function () {
    ...
    $("#ODDPhone").bind("keyup click", function () {
        if ($("#ODDPhone").val().length > 1) {
            var arr = new Array();
            arr[0] = $("#ODDPhone").val();
            var result = eval("(" + ReturnValue("BLL_CallCenterDts", "GetPhone", arr, Address) + ")");
            var html = "";
    ...
    $("#ODDName").bind("keyup click", function () {
        if ($("#ODDName").val().length > 1) {
            var arr = new Array();
            arr[0] = $("#ODDName").val();
            var result = eval("(" + ReturnValue("BLL_CallCenterDts", "GetName", arr, Address) + ")");
            var html = "";
            for (var i = 0; i < result.length; i++) {
    ...
    $(document).delegate('li', 'click', function () {
        ...
        if (liPlat == "1") {
            arr[0] = $(this).text();
            arr[1] = "";
            result = eval("(" + ReturnValue("BLL_CallCenterDts", "GetComData", arr, Address) + ")");
        }
        else {
            arr[0] = "";
            arr[1] = $(this).text();
        ...
        $("#Custlinkman").val(result[0].ODD_LinkMan);
        $("#CustlinkPhone").val(result[0].ODD_LinkPhone);
        ...
        $("#CustAddress").val(result[0].ODD_Address);
        ...
        //liger.get("IsBill").setText(result[0].ODD_IsBill);
    });
...
function displayShowByFromPage() {
    if (Code != "") {
        if (fromPage == 'CustomerReturns_A') {
            ...
            $('#CustPhone1').removeAttr('disabled');
            $('#CustAddress').removeAttr('disabled');
            //$("#CustNameKey").removeAttr("disabled");
            document.getElementById("SearchDiv").style.display = "none";
            document.getElementById("SearchDiv1").style.display = "none";
...
function GetData() {
    var arr = new Array();
    arr[0] = Code;
    var Data = eval(ReturnValue("BLL_CallCenterDts", "GetCallCenterDts", arr, Address));
    if (Data.length > 0) {
        if (Data[0].Cust_Code != "") {
            $("#CustName").val(Data[0].Cust_Name);
            custState = Data[0].Cust_State;
            custBillNumber = Data[0].Cust_BillNumber;
            if (custState == "异常退回") {
                disabledByID("CustName,Custlinkman,CustlinkPhone,Cust_KFVoice,Cust_Source,Cust_WH_Remark");
                $("#dw").hide();
                $("#jfProvince").ligerComboBox({
                    value: Data[0].Cust_BelongProvinceCode, disabled: true
                });
                $("#jfCity").ligerComboBox({
                    value: Data[0].Cust_BelongCityCode, disabled: true
                })
                $("#Province").ligerComboBox({
                    value: Data[0].Cust_ProvinceCode, disabled: true
                });
                $("#City").ligerComboBox({
                    value: Data[0].Cust_CityCode, disabled: true
                });
                $("#District").ligerComboBox({
                    value: Data[0].Cust_CountyCode, disabled: true
                });
                $("#Cust_IsBill").ligerComboBox({
                    value: Data[0].Cust_IsBill, disabled: true
                });
                $("#Cust_Nature").ligerComboBox({
                    value: Data[0].Cust_Nature, disabled: true
                });
                $("#tr_Cust_WH_Remark").hide();
            }
            else {
                $("#jfProvince").ligerComboBox({
            ...
            //$("#CustNameKey").attr("disabled", "disabled");
            ...
            if (Data[0].Cust_State == "退回" || Data[0].Cust_State == "异常退回") {
...
//异常退回 禁用其他文本下拉框
function disabledByID(ids) {
    var arrid = ids.split(',');
    for (var i = 0; i < arrid.length; i++)
        $("#" + arrid[i]).attr("disabled", "disabled");
}

function SaveData() {
    //if (custState == "异常退回")
    //{
    //    var fillBillNuber = $("#Cust_BillNumber").val();
    //    if(fillBillNuber == custBillNumber)
    //    {
    //        $.ligerDialog.warn("计费号码未修改，请修改计费号码！");
    //        return;
    //    }
    //}
    if (VaildData() == true) {
        //var arrLink = new Array();
        //arrLink[0] = $("#CustlinkPhone").val();
        //var existLinkPhone = ReturnValue("BLL_CallCenterDts", "CheckExistLinkPhone", arrLink, Address);
        //if (existLinkPhone == "true")
        //{
        //    $.ligerDialog.error("联系电话已存在，请将是否计费改成否。");
        //    return;
        //}
        var arr = new Array();
        arr[0] = Code;
        ...
        arr[4] = $("#CustPhone1").val();
        arr[5] = $("#Custlinkman").val();
        arr[6] = $("#CustlinkPhone").val();
        arr[7] = $("#value5").val();
        arr[8] = $("#Province").val();
        arr[9] = $("#value6").val();
        arr[10] = $("#City").val();
...
var CustomMethod = {
    ...
    UpdateSuccess: function (text) {
        ...
        //else if (text == "-1") {
        //    $.ligerDialog.error("联系电话已经存在，请将是否计费<br/>改成否!");
        //} else {
...
function validatejfhm(val,code) {
    ...
    arr[0] = val;
    arr[1] = code;
    var Data = JSON.parse(ReturnValue("BLL_CustomerReturnsDts", "GetCustomerListByBillNumber", arr, Address));
    if (Data.length > 0) {
...
function VaildData() {
    ...
    if ($("#jfProvince").val() == "") {
        $.ligerDialog.warn('归属地省不能为空！');
        $("#jfProvince").focus();
        return false;
    }
    if ($("#jfCity").val() == "") {
        $.ligerDialog.warn('归属地市不能为空！');
        $("#jfCity").focus();
        return false;
    }
    if ($("#District").val() == "") {
