...
function InitData() {
    var arr = new Array();
    arr[0] = Code;
    var oneData = eval(ReturnValue("BLL_CustomerService", "GetCustomerByCode", arr, Address));
    ...
    var phone = '02582230315';
    if (oneData[0].Cust_ProvinceName == "宁夏省") {
        phone = "09515022525";
    }
    else if (oneData[0].Cust_ProvinceName == "云南省") {
        phone = "087165814441";
    }
    else if (oneData[0].Cust_ProvinceName == "贵州省") {
        phone = "0851-85554415";
    }
    else if (oneData[0].Cust_ProvinceName == "山东省")//8.2添加by：zh
    {
        phone = "4000085315";
    }
    var message = oneData[0].Cust_Linkman + ": 您好! " + oneData[0].Cust_Name + " 已开通消法维权通（旺铺助手诚信通）快速处理投诉平台（www.315cc.com.cn）或通过微信关注\"中国消费者\"查询投诉，用户名：" + oneData[0].Cust_BillNumber + "，密码：123,请妥善保管,若有投诉单会另行短信通知。询:4000025315/4000085315。";
    if (oneData[0].Cust_ProvinceName == "河南省") {
        message = oneData[0].Cust_Linkman + ": 您好! " + oneData[0].Cust_Name + " 的“消法维权通”用户名：" + oneData[0].Cust_BillNumber + ",密码：123,请妥善保管,您可以登陆投诉处理平台（http://at315.at315.com）或通过微信关注“中国消费者”查询投诉，若有投诉单会另行短信通知。询:4000025315";
    }
    if (oneData[0].Cust_ProvinceName == "山东省")//8.27添加by：zh
    {
        message = oneData[0].Cust_Linkman + ": 您好! " + oneData[0].Cust_Name + " 的“消法维权通”用户名：" + oneData[0].Cust_BillNumber + ",密码：123,请妥善保管,本业务18元/月,在该固话中收取。您可以登陆投诉处理平台（http://www.315cc.com.cn）或通过微信关注“中国消费者”查询投诉，若有投诉单会另行短信通知。询:4000085315";
    }
    if (oneData[0].Cust_ProvinceName == "湖南省")//9.19添加by：zh
    {
        message = oneData[0].Cust_Linkman + ": 您好! " + oneData[0].Cust_Name + " 的“消法维权通”用户名：" + oneData[0].Cust_BillNumber + ",密码：123,请妥善保管,尽快登陆平台修改初始密码。您可以登陆投诉处理平台（http://at315.at315.com）或通过微信关注“中国消费者”查询投诉，若有投诉单会另行短信通知。询:4000085315";
    }
    if (oneData[0].Cust_ProvinceName == "安徽省")//9.19添加by：zh
    {
        message = oneData[0].Cust_Linkman + ": 您好! " + oneData[0].Cust_Name + " 的“消法维权通”用户名：" + oneData[0].Cust_BillNumber + ",密码：123,请妥善保管,尽快登陆平台修改初始密码。您可以登陆投诉处理平台（http://at315.at315.com）或通过微信关注“中国消费者”查询投诉，若有投诉单会另行短信通知。询:4000085315";
    }
    if (oneData[0].Cust_ProvinceName == "江苏省")//9.19添加by：zh
    {
        message = oneData[0].Cust_Linkman + ": 您好! " + oneData[0].Cust_Name + " 已开通消法维权通（旺铺助手诚信通）快速处理投诉平台（www.315cc.com.cn）或通过微信关注\"中国消费者\"查询投诉，用户名：" + oneData[0].Cust_BillNumber + "，密码：123,请妥善保管,若有投诉单会另行短信通知。询:4000025315/4000085315。";
    }
   ...
   // CustjfAddress = oneData[0].Cust_BelongProvinceName + oneData[0].Cust_BelongCityName;
...
function Exam() {
    if (State == "Exam") {
        ...
        var result = ReturnValue("BLL_CustomerService", "Pass", arr, Address);
        if (result == "true") {
            if (arr[1] == "0") {
                layer.msg("审核成功");
                // 修改营销成功
                ...
                ReturnValue("BLL_CustomerService", "UpAlreadyUse", arr1, Address);
...
function CreateAndSendMSG() {
    var result = false;
    $.ajax({
        url: Address + 'Aspx/SendMsg.aspx?method=SendMsgs&Message=' + encodeURI($("#txtMessage").val()) + '&Phone=' + $("#CustLinkPhone").html(),
        cache: false,
        async: false,
...
function InitCount() {
    ...
    arr[3] = BillNumber;
    arr[4] = CustAddress;
    var Method = "GetSimilarInfoCount";
...
function Search() {
    ...
    if (searchtype == "0") {//全部
        ...
        var BLL = "BLL_CustomerService";
        var search_result = JSON.parse(ReturnValue("BLL_CustomerService", "GetSimilarInfo", arr, Address));
        MaxCount = ReturnValue("BLL_CustomerService", "GetSimilarCount", arr, Address)
        MaxPage = (MaxCount % PageNum == 0 ? MaxCount / PageNum : Math.ceil((MaxCount / PageNum)));
        objdata = search_result;
        gridManager = $("#grid").ligerGrid({
            //url: "../Ashx/GetPublicData.ashx?Data=" + Method + "臡" + BLL + "臡" + encodeURI(arrStr)
            data: JSON.parse(search_result[0].TabStr.replace('\\', ''))
        });
    }
    else
        if (searchtype == 1) {//名称包含关键字
            arr[0] = Code;
            arr[1] = KeyName;
            arr[2] = PageIndex;
            arr[3] = PageNum;
            var Method = "GetNameContainsKeySimilarInfo";
            var BLL = "BLL_CustomerService";
            var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
            ...
            if (searchtype == 2) {//计费号码相
                arr[0] = Code;
                arr[1] = BillNumber;
                arr[2] = PageIndex;
                arr[3] = PageNum;
                var Method = "GetBillNoSimilarInfo";
                var BLL = "BLL_CustomerService";
                var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
                ...
                if (searchtype == 3) {//地址相似
                    arr[0] = Code;
                    arr[1] = CustAddress;
                    arr[2] = PageIndex;
                    arr[3] = PageNum;
                    var Method = "GetCustAddressSimilarInfo";
                    var BLL = "BLL_CustomerService";
                    var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
                    ...
                    if (searchtype == 4) {//联系电话相似
                        ...
                        var arrStr = arr;
                        ReplaceStr(arrStr);
                        var Method = "GetCustPhoneSimilarInfo";
                        var BLL = "BLL_CustomerService";
                        var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
    ...
    // $("#spanCount").html("相似客户：<a href='javascript:ReSearch(0)'>全部(" + MaxCount + "条)</a>&nbsp;&nbsp;" + getGoupInfo(search_result));
