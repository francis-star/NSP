...
function InitControl() {
    ...
    /********权限******/
    GetMenuRight(toptoolbarManager, "2019082811942");
...
function Search() {
    ...
    GridLoadAll("BLL_OriginalData", "GetSSBOriginalData", "GetSSBOriginalDataCount", arr);
}
...
function View() {
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['65%', '70%'],
        content: '../Form/SSB/frm_View.htm?code=' + rows[0].OD_Code
    });
...
function OrigDataDts(OD_Code, OD_TotleCount) {
    if (OD_TotleCount == 0) {
        ...
        top.layer.open({
            ...
            shade: 0.3,
            area: ['50%', '50%'],
            content: '../Form/SSB/frm_OrigDataDts.htm?code=' + OD_Code
        });
    }
}
...
function VaildData(OD_Code, OD_TotleCount) {
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['50%', '50%'],
        content: '../Form/SSB/frm_ValidData.htm?code=' + OD_Code
    });
}
...
function NOVaildData(OD_Code, OD_TotleCount) {
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['50%', '50%'],
        content: '../Form/SSB/frm_NOVaildData.htm?code=' + OD_Code
    });
}
...
function ShowDialog() {
    top.layer.open({
        ...
        shade: 0.3,
        area: ['35%', '68%'],
        content: '../Form/SSB/frm_ImportData.htm',
        end: function () {
            InitData();
        }
    });
...
function ShowInvalidData() {
    top.layer.open({
        ...
        shade: 0.3,
        area: ['35%', '50%'],
        content: '../Form/SSB/frm_DInvalidData.htm',
        end: function () {
            InitData();
        }
    });
...
function DataScreen() {
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['65%', '70%'],
        content: '../Form/SSB/frm_DataScreen.htm?code=' + str,
        end: function () {
            InitData();
        }
    });
...
function MarkAlready() {
    ...
    $.ligerDialog.confirm('确认将此数据状态标记为“已使用”？', function (yes) {
        if (yes) {
            ...
            ReturnValue("BLL_OriginalData", "MarkSSBAlreadyUse", arr, Address, "");
            InitData();
        }
    });
}
...
function DeleteDatas() {
    ...
    $.ligerDialog.confirm('确定删除该数据？', function (yes) {
        if (yes) {
            var arr = new Array();
            arr[0] = rows[0].OD_Code;
            ReturnMethod("BLL_OriginalData", "DeleteSSBDatas", arr, Address, "DeleteSucess");
        }
    });
}
...
function UpdateDatas() {
    ...
    top.layer.open({
        ...
        shade: 0.3,
        area: ['30%', '45%'],
        content: '../Form/SSB/frm_UpdateOrigData.htm?Code=' + rows[0].OD_Code,
        end: function () {
            InitData();
        }
    });
