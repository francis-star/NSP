...
$(function () {
    ...
    $("#ODDPhone").bind("keyup click", function () {
        if ($("#ODDPhone").val().length > 1) {
            var arr = new Array();
            arr[0] = $("#ODDPhone").val();
            var result = eval("(" + ReturnValue("BLL_CallCenterDts", "GetSSBPhone", arr, Address) + ")");
            var html = "";
    ...
    $("#ODDName").bind("keyup click", function () {
        if ($("#ODDName").val().length > 1) {
            var arr = new Array();
            arr[0] = $("#ODDName").val();
            var result = eval("(" + ReturnValue("BLL_CallCenterDts", "GetSSBName", arr, Address) + ")");
            var html = "";
            for (var i = 0; i < result.length; i++) {
    ...
    $(document).delegate('li', 'click', function () {
        ...
        if (liPlat == "1") {
            arr[0] = $(this).text();
            arr[1] = "";
            result = eval("(" + ReturnValue("BLL_CallCenterDts", "GetSSBComData", arr, Address) + ")");
        }
        else {
            arr[0] = "";
            arr[1] = $(this).text();
...
function InitControl() {
    ...
    var IsBillData = eval("[{ text: '是2', id: '是' },{ text: '否', id: '否' }]");
    $("#Cust_IsBill").ligerComboBox({
        data: IsBillData,
        textField: "text",
        valueField: "id",
    ...
    var OperatorData = eval("[{ text: '中国移动', id: '中国移动' },{ text: '中国联通', id: '中国联通' },{ text: '中国电信', id: '中国电信' }]");
    $("#operator").ligerComboBox({
        data: OperatorData,
        textField: "text",
        valueField: "id",
        valueFieldID: "value10",
        value: OperatorData[0].id,
        width: 140,
        cancelable: false,
        isMultiSelect: false
    });
...
function GetData() {
    var arr = new Array();
    arr[0] = Code;
    var Data = eval(ReturnValue("BLL_CallCenterDts", "GetSSBCallCenterDts", arr, Address));
    if (Data.length > 0) {
        if (Data[0].Cust_Code != "") {
...
function validatejfhm(val,code) {
    ...
    arr[0] = val;
    arr[1] = code;
    var Data = JSON.parse(ReturnValue("BLL_CustomerReturnsDts", "GetSSBCustomerListByBillNumber", arr, Address));
    if (Data.length > 0) {
