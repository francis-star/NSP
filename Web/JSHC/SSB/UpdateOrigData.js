...
function InitData() {
    var arr = new Array();
    arr[0] = Code;
    OrginData = eval(ReturnValue("BLL_OriginalData", "GetSSBOriginData", arr, Address));
}
...
function UpdateDatas() {
    if (validata()) {
        ...
        ReturnMethod("BLL_OriginalData", "UpdateSSBDatas", arr, Address, "UpdateDatas");
    }
}
...
function validata() {
    if ($("#Province").ligerComboBox().getValue() == "") {
    ...
    if ($("#Billing").ligerComboBox().getValue() == "") {
        $.ligerDialog.warn('请补填写完整信息！');
        $("#Billing").focus();
        return false;
    }
