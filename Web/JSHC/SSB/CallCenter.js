...
function InitControl() {
    ...
    GetMenuRight(toptoolbarManager, "2019082811943");
}
...
function Search() {
    ...
    MaxCount = ReturnValue("BLL_CallCenter", "GetSSBDataCount", arr, Address);
    MaxPage = (MaxCount % PageNum == 0 ? MaxCount / PageNum : Math.ceil((MaxCount / PageNum)));

    var arrStr = arr;
    ReplaceStr(arrStr);
    var Method = "GetSSBCallCenterData";
...
function Add() {
    top.layer.open({
        ...
        shade: 0.3,
        area: ['70%', '80%'],
        content: '../../Form/SSB/frm_CallCenterDts.htm?Code=',//iframe的url
        end: function () {
            Search();
        }
    });
    ...
    if (row) {
        if (row.Cust_State == "待审" || row.Cust_State == "退回") {
            top.layer.open({
                ...
                shade: 0.3,
                area: ['70%', '80%'],
                content: '../../Form/SSB/frm_CallCenterDts.htm?fromPage=CallCenter_A&Code=' + row.Cust_Code, //iframe的url
                end: function () {
                    Search();
                }
            });
    ...
    if (row) {
        ...
        $.ligerDialog.confirm('确定删除？', function (yes) {
            if (yes) {
                var arr = new Array();
                arr[0] = row.Cust_Code;
                var Data = ReturnValue("BLL_CallCenter", "DeleteSSBCallCenterData", arr, Address);
                if (Data == "true") {
