...
function InitControl() {
    ...
    var TSSourceData = [{ id: 1, text: "省号百" }, { id: 2, text: "省投" }, { id: 3, text: "区域中心" }, { id: 4, text: "南京号百" }, { id: 5, text: "用户" }, { id: 6, text: "客户经理" }, { id: 7, text: "地市号百" }, { id: 8, text: "地市营业厅" }, { id: 9, text: "公司" }];
    ...
    var arr = new Array();
    arr[0] = CustCode;
    var Result = eval("(" + ReturnValue("BLL_CustomerReturnsDts", "GetXFBBilllist", arr, Address) + ")");
    gridManager = $("#grid").ligerGrid({
        columns: [
            { display: '年度', name: 'BL_Year', minWidth: 40 },
            { display: '总计', name: 'BL_TotalMoney', minWidth: 40 },
            { display: '一月', name: 'BL_One', minWidth: 40 },
            { display: '二月', name: 'BL_Two', minWidth: 40 },
            { display: '三月', name: 'BL_Three', minWidth: 40 },
            { display: '四月', name: 'BL_Four', minWidth: 40 },
            { display: '五月', name: 'BL_Five', minWidth: 40 },
            { display: '六月', name: 'BL_Six', minWidth: 40 },
            { display: '七月', name: 'BL_Seven', minWidth: 40 },
            { display: '八月', name: 'BL_Eight', minWidth: 40 },
            { display: '九月', name: 'BL_Nine', minWidth: 40 },
            { display: '十月', name: 'BL_Ten', minWidth: 40 },
            { display: '十一月', name: 'BL_Eleven', minWidth: 40 },
            { display: '十二月', name: 'BL_Twelve', minWidth: 40 }
        ],
        pageSize: 10, width: '95%', height: '150px',
        pageSizeOptions: [10], enabledSort: false,
        userPager: false, isScroll: false,
    ...
    //$("#Cust_TSPhone").attr("disabled", true);
...
function InitData() {
    var arr = new Array();
    arr[0] = CustCode;
    ReturnMethod("BLL_CustomerReturnsDts", "GetXFBDetails", arr, Address, "GetDataSuccess");
...
function change() {
    if ($("#Cust_State").val() == "退订") {
        ...
        $("#Cust_TSNatrue").ligerTextBox().setEnabled();
        $("#Cust_TSSource").ligerTextBox().setEnabled();
        //$("#Cust_TSPhone").attr("disabled", false);
...
function Exam() {
    if (VaildData()) {
        ...
        ReturnMethod("BLL_CustomerReturnsDts", "ModXFBState", arr, Address, "SaveQuitSuccess");
    }
}
