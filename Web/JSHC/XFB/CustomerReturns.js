...
function InitControl() {
                ...
                text: '处理', id: 'btnDeal', icon: 'config', click: function () {
                    ...
                    if (row) {
                        top.layer.open({
                            ...
                            shade: 0.3,
                            area: ['90%', '90%'],
                            content: '../../Form/XFB/frm_CustomerReturnsDts.htm?Code=' + row.Cust_Code, //iframe的url
                            end: function () {
                                Search();
                            }
                        });
                ...
                text: '编辑', id: 'btnModify', icon: 'modify', click: function () {
                    ...
                    if (row) {
                        top.layer.open({
                            ...
                            shade: 0.3,
                            area: ['90%', '90%'],
                            content: '../../Form/XFB/frm_CallCenterDts.htm?fromPage=CustomerReturns_A&Code=' + row.Cust_Code, //iframe的url
                            end: function () {
                                Search();
                            }
                        });
                ...
                text: '查看', id: "btnView", icon: 'view', click: function () {
                    ...
                    if (row) {
                        top.layer.open({
                            ...
                            shade: 0.3,
                            area: ['90%', '90%'],
                            content: '../../Form/XFB/frm_CustomerReturnscheck.htm?Code=' + row.Cust_Code, //iframe的url
                            end: function () {
                                Search();
                            }
                        });
    ...
    GetMenuRight(toptoolbarManager, "2019062711945");

    gridManager = $("#grid").ligerGrid({
        columns: [
            { display: '客户名称', name: 'Cust_Name', minWidth: 150 },
            { display: '联系电话', name: 'Cust_Phone', minWidth: 100 },
...
function OutExcel() {
    ...
    arr[15] = $("#tbPlace").val();
    var data = ReturnValue("BLL_CustomerReturns", "DtXFBToExcel", arr, Address);
    if (data == "true")
...
function Search() {
    ...
    MaxCount = ReturnValue("BLL_CustomerReturns", "GetXFBNum", arr, Address);
    MaxPage = (MaxCount % PageNum == 0 ? MaxCount / PageNum : Math.ceil((MaxCount / PageNum)));

    var arrStr = arr;
    ReplaceStr(arrStr);
    var Method = "GetXFBCust";
