...
function SaveOrganization() {
    if (validata()) {
        ...
        if (result == "true") {
            ...
            ReturnMethod("BLL_OriginalData", "PlImportXFBOrignData", arr, Address, "SaveDataSuccess");
        }
        else {
            ...
            if (strValue.length > 1) {
                if (strValue[0] == "special") {
                    ...
                    $("#tbPC_Name").val("");
                } else if (strValue[0] == "remark") {
                    $.ligerDialog.warn("筛选数据在第" + strValue[1] + "行【" + strValue[2] + "】出现错误，备注超过500！");
                    $("#tbPC_Name").val("");
                }
                else {
