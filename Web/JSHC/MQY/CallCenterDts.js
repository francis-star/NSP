...
$(function () {
    ...
    $("#ODDPhone").bind("keyup click", function () {
        if ($("#ODDPhone").val().length > 1) {
            var arr = new Array();
            arr[0] = $("#ODDPhone").val();
            var result = eval("(" + ReturnValue("BLL_CallCenterDts", "GetMQYPhone", arr, Address) + ")");
            var html = "";
    ...
    $("#ODDName").bind("keyup click", function () {
        if ($("#ODDName").val().length > 1) {
            var arr = new Array();
            arr[0] = $("#ODDName").val();
            var result = eval("(" + ReturnValue("BLL_CallCenterDts", "GetMQYName", arr, Address) + ")");
            var html = "";
            for (var i = 0; i < result.length; i++) {
    ...
    $(document).delegate('li', 'click', function () {
        ...
        if (liPlat == "1") {
            arr[0] = $(this).text();
            arr[1] = "";
            result = eval("(" + ReturnValue("BLL_CallCenterDts", "GetMQYComData", arr, Address) + ")");
        }
        else {
            arr[0] = "";
            arr[1] = $(this).text();
...
function GetData() {
    var arr = new Array();
    arr[0] = Code;
    var Data = eval(ReturnValue("BLL_CallCenterDts", "GetMQYCallCenterDts", arr, Address));
    if (Data.length > 0) {
        if (Data[0].Cust_Code != "") {
