...
function InitCount() {
    ...
    var Method = "GetMQYSimilarInfoCount";
...
function InitData() {
    var arr = new Array();
    arr[0] = Code;
    var oneData = eval(ReturnValue("BLL_CustomerService", "GetMQYCustomerByCode", arr, Address));
    ...
    var message = "欢迎加入省私个协微信平台，打开微信“添加朋友”搜“jsssgxh”或“江苏省私个协会”关注。询：02583729711";
...
function Exam() {
    if (State == "Exam") {
        ...
        var result = ReturnValue("BLL_CustomerService", "PassMQY", arr, Address);
        if (result == "true") {
            if (arr[1] == "0") {
                layer.msg("审核成功");
                // 修改营销成功
                ...
                ReturnValue("BLL_CustomerService", "UpMQYAlreadyUse", arr1, Address);
...
function CreateAndSendMSG() {
    var result = false;
    $.ajax({
        url: Address + '../../Aspx/SendMsg.aspx?method=SendMsgsMQY&Message=' + encodeURI($("#txtMessage").val()) + '&Phone=' + $("#CustLinkPhone").html(),
        cache: false,
        async: false,
...
function Search() {
    ...
    if (searchtype == "0") {//全部
        ...
        arr[5] = BillNumber;
        arr[6] = CustAddress;
        var search_result = JSON.parse(ReturnValue("BLL_CustomerService", "GetMQYSimilarInfo", arr, Address));
        ...
        arr[2] = PageIndex;
        arr[3] = PageNum;
        var Method = "GetMQYNameContainsKeySimilarInfo";
        var BLL = "BLL_CustomerService";
        var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
        ...
        arr[2] = PageIndex;
        arr[3] = PageNum;
        var Method = "GetMQYBillNoSimilarInfo";
        var BLL = "BLL_CustomerService";
        var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
        ...
        arr[2] = PageIndex;
        arr[3] = PageNum;
        var Method = "GetMQYCustAddressSimilarInfo";
        var BLL = "BLL_CustomerService";
        var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
        ...
        var arrStr = arr;
        ReplaceStr(arrStr);
        var Method = "GetMQYCustPhoneSimilarInfo";
        var BLL = "BLL_CustomerService";
        var search_result = JSON.parse(ReturnValue(BLL, Method, arr, Address));
